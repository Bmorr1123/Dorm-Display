...
class Bouncer:
    ...
    def __init__(self, center, velocity, image):
        ...
        if not (Bouncer.width and Bouncer.height):
            print("WIDTH AND HEIGHT HASN'T BEEN SET")
    ...
    def tick(self, delta):
        ...
        if self.x() < 0 or Bouncer.width < self.x() + self.size[0]:
            self.velocity.x *= -1
            ...
            self.position.x = min(max(0, self.x()), Bouncer.width)

        if self.y() < 0 or Bouncer.height < self.y() + self.size[1]:
            self.velocity.y *= -1
            ...
            self.position.y = min(max(0, self.y()), Bouncer.height)
